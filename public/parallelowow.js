  ...
  registerPaint(
    ...
    class {
      ...
      paint(ctx, geom, properties) {
        let tileWidth =
          Number(properties.get("--parallelowow-tile-width")) || 56;
        let baseColor =
          properties.get("--parallelowow-base-color").toString().trim() ||
          "#c9f";
        let colorStep =
          Number(properties.get("--parallelowow-color-step")) || -3;
        let probability =
          Number(properties.get("--parallelowow-probability")) || 0.33;
        let strokeWeight =
          Number(properties.get("--parallelowow-stroke-weight")) || 0.5;
        ...
        const outerRadius =
          geom.width > geom.height ? geom.width * 2 : geom.height * 2;
        ...
        for (let y = -1; y < yTiles; y++) {
          ...
          colors = colors.map((colorKey) =>
            this.adjustBrightness(colorKey, colorStep)
          );
        }
      }
      ...
      adjustBrightness(colorString, amt) {
        let rgbString = this.isValidHexColor(colorString)
          ? this.hexToRgb(colorString)
          : colorString;
        rgbString = rgbString
          .replace(/rgba?\(/g, "")
          .replace(/\)/g, "")
        ...
        return rgbString.indexOf("rgba") !== -1
          ? `rgba(${rgbParts.join(",")})`
          : `rgb(${rgbParts.join(",")})`;
      }
    }
